...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, crf_mask=None, labels=None, head_mask=None):
        outputs = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, head_mask=head_mask)
        if self.output_attentions:
            all_attentions, sequence_output, _ = outputs
        else:
...
class Bert_CRF(BertPreTrainedModel):
    ...
    ```
    """
    def __init__(self, config, bio_num_labels=3, constraints=None, include_start_end_transitions=True, output_attentions=False, keep_multihead_output=False):
        ...
        self.bert = BertModel(config, output_attentions=output_attentions,
                                      keep_multihead_output=keep_multihead_output)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.bio_classifier = nn.Linear(config.hidden_size, self.bio_num_labels)
        self.apply(self.init_bert_weights)
        self.post = ConditionalRandomField(num_tags=bio_num_labels, constraints=constraints, include_start_end_transitions=include_start_end_transitions)
        self.criterion = nn.CrossEntropyLoss()

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, crf_mask=None, bio_labels=None, head_mask=None, check=False):
        self.bert.eval()
        with torch.no_grad():
            outputs = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, head_mask=head_mask)
        if self.output_attentions:
        ...
        crf_result = self.post(bio_logits, bio_labels, crf_mask, check=check)

        # Train: BIO Labels are present
        if bio_labels is not None:
            if check:
                crf_loss, bio_score, crf_pred = crf_result
                crf_loss = crf_loss / crf_mask.sum()

                return crf_loss, bio_score, crf_pred
