...
class ConditionalRandomField(torch.nn.Module):
    ...
    def viterbi_tags(self,
        ...
        for prediction, prediction_mask in zip(logits, mask):
            ...
            viterbi_path, viterbi_score = self.viterbi_decode(tag_sequence[:(sequence_length + 2)].cuda(), transitions.cuda())
            # Get rid of START and END sentinels and append.
            viterbi_path = viterbi_path[1:-1]
            best_paths['output'].append(viterbi_path)
            best_paths['loss'].append(viterbi_score.item())
        """
        ...
        best_paths = self._viterbi_decode(constrained_transitions.cuda(), logits.cuda(), mask.cuda())
        ...
        for timestep in range(1, sequence_length):
            ...
            if tag_observations[timestep - 1] != -1:
                if transition_matrix[tag_observations[timestep - 1], observation] < -10000:
                    warnings.warn("The pairwise potential between tags you have passed as "
    ...
    def _viterbi_decode(self, transitions, emissions, masks):
        ...
        masks = masks.transpose(0, 1).byte()
        device = masks.device
        ...
        score = (self.start_transitions*self._constraint_mask[start_tag, :num_tags]
                - 10000.0 * (1 - self._constraint_mask[start_tag, :num_tags].detach())
                + emissions[0])
        ...
        for emission, mask in zip(emissions[1:], masks[1:]):
            ...
            next_score = broadcast_score + transitions
            ...
            # shape: (batch_size, num_tags)
            #print(mask.shape, next_score.shape, score.shape)

            score = torch.where(mask.unsqueeze(1), next_score+emission, score)
            history.append(indices)

        # End transition score
        # shape: (batch_size, num_tags)
        score += self.end_transitions*self._constraint_mask[:num_tags, end_tag] -10000.0 * (1 - self._constraint_mask[:num_tags, end_tag].detach())

        # Now, compute the best path for each sample

        best_paths = {'output':[],'loss':[]}
        best_scores, best_last_tags = score.max(dim=-1)

        for idx, seq_length in enumerate( masks.long().sum(dim=0)-1 ):
            ...
            for hist in reversed(history[:seq_length]):
                best_last_tag = hist[idx][ best_tags[-1] ]
                best_tags.append(best_last_tag.item())

            # Reverse the order because we start from the last timestep
            best_tags.reverse()
            best_paths['output'].append(best_tags)
            best_paths['loss'].append(best_scores[idx])

        return best_paths

    def matrix(self, preds, golds):
        golds = golds.cpu().tolist()
        f1 = 0
        for i,(pred, gold) in enumerate(zip(preds, golds)):
            f1 += f1_score(pred, gold[:len(pred)], average='macro')
        return f1
    def forward(self, inputs, tags = None, mask= None, check =False):
        """
        ...
        if(check):
            ...
            pred = self.viterbi_tags(inputs,mask)['output']

            return (loss(log_numerator, log_denominator)), self.matrix(pred, tags), pred

        elif(tags is not None):
            # if there is tags, output loss
            loss = nn.MSELoss()
            log_denominator = self._input_likelihood(inputs, mask)
            log_numerator = self._joint_likelihood(inputs, tags, mask)

            return (loss(log_numerator, log_denominator))

        else:
            # if there isn't tags, output best path and prob
            return self.viterbi_tags(inputs,mask)
